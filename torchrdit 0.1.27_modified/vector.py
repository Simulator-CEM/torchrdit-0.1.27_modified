...
class FourierExpansionManager:
    ...
    def __init__(
        ...
        *,
        truncation: str = "PARALLELOGRAMIC",
    ) -> None:
        self._lattice_vectors = lattice_vectors
        self._truncation = truncation.upper()
...
def _generate_circular_coefficients(
    reciprocal_vectors: LatticeVectors,
    approximate_num_terms: int,
) -> torch.Tensor:
    """Enumerate Fourier coefficients using circular truncation."""
    device = reciprocal_vectors.u.device
    dtype = reciprocal_vectors.u.dtype
    g = torch.arange(
        -approximate_num_terms // 2,
        approximate_num_terms // 2 + 1,
        device=device,
        dtype=torch.int64,
    )
    if g.numel() == 0:
        g = torch.zeros(1, dtype=torch.int64, device=device)
    ...
    magnitude = torch.linalg.norm(vectors, dim=-1)
    cross = torch.abs(_cross_product(reciprocal_vectors.u, reciprocal_vectors.v))
    max_magnitude = torch.sqrt(
        torch.tensor(float(approximate_num_terms), device=device, dtype=dtype) * cross / math.pi
    )
    ...
    coeffs = torch.stack([G1, G2], dim=-1)
    order = torch.argsort(magnitude, stable=True)
    return coeffs[order]
...
def generate_expansion(
    ...
    *,
    ...
    trunc_upper = truncation.upper()
    if trunc_upper == "CIRCULAR":
        coeffs = _generate_circular_coefficients(reciprocal_vectors, approximate_num_terms)
    elif trunc_upper == "PARALLELOGRAMIC":
        coeffs = _generate_parallelogram_coefficients(reciprocal_vectors, approximate_num_terms)
    else:
        raise ValueError(
            f"Unsupported truncation '{truncation}', expected 'CIRCULAR' or 'PARALLELOGRAMIC'."
        )
...
def _compute_gradient(
    ...
    pad_width = (1, 1, 1, 1)
    arr_expanded = arr.unsqueeze(0).unsqueeze(0)
    arr_padded = F.pad(arr_expanded, pad_width, mode="circular").squeeze(0).squeeze(0)
    grad_u_padded, grad_v_padded = torch.gradient(arr_padded, dim=(-2, -1))
    grad_u = grad_u_padded[..., 1:-1, 1:-1] * arr.shape[-2]
    grad_v = grad_v_padded[..., 1:-1, 1:-1] * arr.shape[-1]
...
def _field_loss_value_jac_and_hessian(
    ...
    """Evaluate loss, gradient, and Hessian."""
    ...
    flat_real = flat_real.detach().requires_grad_(True)
    ...
    return loss_value.detach(), jac.detach(), hessian.detach()


def _field_loss_value_jac_and_hessian_complex(
    flat_fourier_field: torch.Tensor,
    expansion: Expansion,
    primitive_lattice_vectors: LatticeVectors,
    fourier_penalty_weights: torch.Tensor,
    target_field: torch.Tensor,
    ...
    """Compute loss, Jacobian, and Hessian for complex Fourier field.
    
    For complex optimization, we use Wirtinger calculus. Since the loss is real,
    we compute gradients with respect to the conjugate of the parameters.
    """
    assert flat_fourier_field.ndim == 1
    field_shape = (expansion.num_terms, 2)
    
    # Work with real-valued representation for autograd
    # Convert: [c0, c1, ...] -> [re(c0), im(c0), re(c1), im(c1), ...]
    flat_real = torch.view_as_real(flat_fourier_field).reshape(-1)
    num_complex = flat_fourier_field.numel()
    
    def loss_fn(real_vec: torch.Tensor) -> torch.Tensor:
        """Loss as a function of real vector."""
        complex_vec = torch.view_as_complex(real_vec.reshape(num_complex, 2))
        fourier_field = complex_vec.reshape(field_shape)
        return _field_loss(
            fourier_field,
            expansion=expansion,
            primitive_lattice_vectors=primitive_lattice_vectors,
    ...
    # Compute loss
    flat_real_grad = flat_real.detach().requires_grad_(True)
    loss_value = loss_fn(flat_real_grad)
    
    # Compute Jacobian (gradient) in real space
    jac_real = torch.autograd.grad(loss_value, flat_real_grad, create_graph=True)[0]
    
    # Compute Hessian in real space using functional API (more efficient)
    # Need to detach and recompute for clean Hessian calculation
    flat_real_clean = flat_real.detach().requires_grad_(True)
    hessian_real = torch.autograd.functional.hessian(loss_fn, flat_real_clean)
    
    # Convert gradient to complex: conj(dL/dz) = dL/dz* (Wirtinger derivative)
    # For real loss: dL/dz* = (dL/d(re) + i*dL/d(im))/2
    # But we want the form compatible with FMMAX's solve: x - H^{-1} @ conj(grad)
    # So we return the gradient in complex form
    jac_real_reshape = jac_real.reshape(num_complex, 2)
    jac_complex = torch.view_as_complex(jac_real_reshape)
    
    return loss_value.detach(), jac_complex.detach(), hessian_real.detach()
...
class TangentFieldGenerator:
        ...
        *,
        ...
        Args:
            ...
            scheme (str): Name of the Fast Fourier Factorization tangent scheme. Supported
                ...
                * ``"POL_FOURIER"`` – POL scheme with Fourier-domain regularisation only.
                * ``"NORMAL_FOURIER"`` – NORMAL scheme with Fourier-domain penalty.
                * ``"JONES_FOURIER"`` – JONES scheme with Fourier-domain penalty.
                * ``"JONES_DIRECT_FOURIER"`` – JONES_DIRECT scheme with Fourier penalty.
    ...
    def _scheme_dispatch(
        ...
        *,
        ...
        def _pol_fourier(arr: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            field = self._compute_field(
                arr,
                use_jones_direct=False,
                fourier_loss_weight=fourier_loss_weight,  # 修复：使用外部参数，可调节
                smoothness_loss_weight=0.0,
                steps=steps,
            )
            return field[..., 0], field[..., 1]

        def _normal_fourier(arr: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            field = self._compute_field(
                arr,
                use_jones_direct=False,
                fourier_loss_weight=fourier_loss_weight,  # 修复：使用外部参数，可调节
        ...
        def _jones_fourier(arr: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            field = self._compute_field(
                arr,
                use_jones_direct=False,
                fourier_loss_weight=fourier_loss_weight,  # 修复：使用外部参数，可调节
        ...
        def _jones_direct_fourier(arr: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            field = self._compute_field(
                arr,
                use_jones_direct=True,
                fourier_loss_weight=fourier_loss_weight,  # 修复：使用外部参数，可调节
        ...
        return {
            ...
            POL_FOURIER: _pol_fourier,
            NORMAL_FOURIER: _normal_fourier,
            JONES_FOURIER: _jones_fourier,
            JONES_DIRECT_FOURIER: _jones_direct_fourier,
        }
    ...
    def _compute_field(
        ...
        *,
        ...
        # 融合11.12版本：移除归一化，使调参范围更直观（0-2而非0-20）
        # 这样设置smoothness_weight=1.35就直接生效，无需乘以225
        ...
        dummy_grad = torch.ones((1, 1, 2), dtype=torch.complex64, device=grad.device)
        ...
        # Newton iteration for tangent field optimization
        # FMMAX: flat_fourier_field -= jnp.linalg.solve(hessian, jac.conj())
        flat_fourier_field = fourier_field.reshape(-1)
        
        for _ in range(max(steps, 1)):
            _, jac_complex, hessian_real = _field_loss_value_jac_and_hessian_complex(
                flat_fourier_field,
                expansion=expansion,
                primitive_lattice_vectors=primitive_lattice_vectors,
                fourier_penalty_weights=fourier_penalty_weights,
            ...
            # Convert jac_complex.conj() to real representation for solving
            # conj(a + bi) = a - bi, so we negate imaginary parts
            # Need to resolve_conj() for PyTorch's conjugate view behavior
            jac_conj_real = torch.view_as_real(jac_complex.conj().resolve_conj()).reshape(-1)
            
            # Solve: delta = H^{-1} @ conj(jac)
            # Add small damping for numerical stability
            damping = 1e-8
            identity = torch.eye(
                hessian_real.shape[0],
                dtype=hessian_real.dtype,
                device=hessian_real.device,
            )
            hessian_damped = hessian_real + damping * identity
            
            # Solve the linear system
            delta_real = torch.linalg.solve(hessian_damped, jac_conj_real)
            
            # Convert delta back to complex and update
            delta_complex = torch.view_as_complex(delta_real.reshape(-1, 2))
            flat_fourier_field = flat_fourier_field - delta_complex
...
def compute_field_pol(
    ...
    *,
    ...
    smoothness_loss_weight: float = SMOOTHNESS_LOSS_WEIGHT,
    steps: int = 1,
    truncation: str = "PARALLELOGRAMIC",
) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    return generator.compute(mask, XO, YO, scheme=POL)
...
def compute_field_normal(
    ...
    *,
    ...
    return generator.compute(mask, XO, YO, scheme=NORMAL)
...
def compute_field_jones(
    ...
    *,
    ...
    return generator.compute(mask, XO, YO, scheme=JONES)
...
def compute_field_jones_direct(
    ...
    *,
    fourier_loss_weight: float = 0.0,
    smoothness_loss_weight: float = SMOOTHNESS_LOSS_WEIGHT_JONES_DIRECT,
    steps: int = 1,
    truncation: str = "PARALLELOGRAMIC",
) -> Tuple[torch.Tensor, torch.Tensor]:
    generator = _default_generator(
    ...
    return generator.compute(mask, XO, YO, scheme=JONES_DIRECT)


VECTOR_FIELD_SCHEMES: Dict[str, Callable[..., Tuple[torch.Tensor, torch.Tensor]]] = {
    POL: compute_field_pol,
    NORMAL: compute_field_normal,
    JONES: compute_field_jones,
    JONES_DIRECT: compute_field_jones_direct,
    POL_FOURIER: lambda *args, **kwargs: compute_field_pol(
        *args, fourier_loss_weight=FOURIER_LOSS_WEIGHT, smoothness_loss_weight=0.0, **kwargs
    ),
    NORMAL_FOURIER: lambda *args, **kwargs: compute_field_normal(
        *args, fourier_loss_weight=FOURIER_LOSS_WEIGHT, smoothness_loss_weight=0.0, **kwargs
    ),
    JONES_FOURIER: lambda *args, **kwargs: compute_field_jones(
        *args, fourier_loss_weight=FOURIER_LOSS_WEIGHT, smoothness_loss_weight=0.0, **kwargs
    ),
    JONES_DIRECT_FOURIER: lambda *args, **kwargs: compute_field_jones_direct(
        *args,
