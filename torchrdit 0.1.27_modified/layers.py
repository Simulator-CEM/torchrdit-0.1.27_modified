...
from torch.fft import fft2, fftshift,fft
import torch
import numpy as np

from .toeplitz_torch import toeplitz_torch
...
class LayerManager:
    ...
    def __init__(self, lattice_t1, lattice_t2, vec_p, vec_q) -> None:
    ...
    def gen_toeplitz_matrix(
        self, layer_index: int, n_harmonic1: int, n_harmonic2: int, param: str = "er", method: str = "FFT"
    ):
        ...
        if param == "er":
            ...
            self.layers[layer_index].kermat_td = self._gen_toeplitz2d_td(
                self.layers[layer_index].ermat, n_harmonic1, n_harmonic2, method
            )
            
            self.layers[layer_index].kermat_dt = self._gen_toeplitz2d_dt(
                self.layers[layer_index].ermat, n_harmonic1, n_harmonic2, method
            )
    ...
    def _gen_toeplitz2d(
        self, input_matrix: torch.Tensor, nharmonic_1: int = 1, nharmonic_2: int = 1, method: str = "FFT"
    ) -> torch.Tensor:
    ...
    def _gen_toeplitz2d_dt(
        ...
        if method == "FFT":
            # compute fourier coefficents of input_matrix for only the last two dimensions
            """
            finput_matrix = fftshift(fftshift(fft2(1/input_matrix), dim=-1), dim=-2) / (n_height * n_width)

            finput_matrix = fftshift(fft(1/input_matrix,dim=-1))
            finput_matrix =tinv(finput_matrix)
            finput_matrix = fftshift(fft(input_matrix,dim=-2))
            finput_matrix=finput_matrix/(n_height * n_width)
            ...
            if finput_matrix.ndim == 2:
                return tinv(finput_matrix[p_0 - pf_t, q_0 - qf_t])
            elif finput_matrix.ndim == 3:
                return tinv(finput_matrix[:, p_0 - pf_t, q_0 - qf_t])
            else:
                return None
            ...
            iepsx_fft = fftshift(fft(1.0 / input_matrix, dim=-1), dim=-1) / n_width

    # MATLAB: i_iepsx_mj = zeros(N_x,D_y,N_x)
            dtype=iepsx_fft.dtype
            i_iepsx_mj = torch.zeros(n_height,nharmonic_1,  nharmonic_1,dtype=dtype, device=input_matrix.device)

    # 索引转换: D_x/2+2-N_x : D_x/2+N_x
            x_start = int(n_width // 2 + 1 - nharmonic_1)
            x_end = int(n_width // 2 + nharmonic_1)

    # -------------------------------------------------------
    # Step 2: Build and invert Toeplitz matrices for each column
    # -------------------------------------------------------
            for qq in range(n_height):
              iepsx_m = iepsx_fft[qq,x_start:x_end]
              c = iepsx_m[nharmonic_1 - 1:2 * nharmonic_1-1]
              r = torch.flip(iepsx_m[:nharmonic_1], dims=[0])
              #print(c)
              #print(r)
              toeplitz_mj =toeplitz_torch(c, r)
              i_iepsx_mj[qq,:,  :] = torch.linalg.inv(toeplitz_mj)

    # -------------------------------------------------------
    # Step 3: FFT along y direction
    # MATLAB: epsxy_fft = fftshift(fft(i_iepsx_mj,[],2),2)/D_y
    # -------------------------------------------------------
            epsxy_fft = fftshift(fft(i_iepsx_mj, dim=-3), dim=-3) / n_height

    # MATLAB: epsxy_mnj = epsxy_fft(:,D_y/2+2-N_y:D_y/2+N_y,:)
            y_start = int(n_height // 2 + 1 - nharmonic_2)
            y_end = int(n_height // 2 + nharmonic_2)
            epsxy_mnj = epsxy_fft[ y_start:y_end,:, :]

    # -------------------------------------------------------
    # Step 4: Construct 4D Toeplitz tensor E4
    # -------------------------------------------------------
            E4 = torch.zeros(nharmonic_1, nharmonic_2, nharmonic_1, nharmonic_2,dtype=dtype, device=input_matrix.device)

            for pp in range(nharmonic_1):
             for qq in range(nharmonic_1):
               c = epsxy_mnj[ nharmonic_2 - 1:2 * nharmonic_2 - 1, pp,qq]
               r = torch.flip(epsxy_mnj[ :nharmonic_2,pp, qq], dims=[0])
               E4[ :, pp, :,qq] = toeplitz_torch(c, r)

    # -------------------------------------------------------
    # Step 5: Flatten to 2D EPSxy matrix
    # MATLAB: EPSxy(:,:) = reshape(E4,[N_x*N_y,N_x*N_y])
    # -------------------------------------------------------
            EPSxy = E4.reshape(nharmonic_1 * nharmonic_2, nharmonic_1 * nharmonic_2)
            ...
            '''
            if input_matrix.ndim == 2:
               finput_matrix = fftshift(fft(1/input_matrix,dim=-2),dim=-2)/n_height
               finput_matrix=tinv(finput_matrix)#[p_0 - pf_t, q_0 - qf_t])
               finput_matrix = fftshift(fft(finput_matrix,dim=-1),dim=-1)/n_width
               return finput_matrix[p_0 - pf_t, q_0 - qf_t]
            elif input_matrix.ndim == 3:
                finput_matrix = fftshift(fft(1/input_matrix,dim=-2),dim=-2)/n_height
                finput_matrix=tinv(finput_matrix)#[:,p_0 - pf_t, q_0 - qf_t])
            ...
            '''
            """
            finput_matrix = fftshift(fft(1/input_matrix,dim=-1),dim=-1)/n_height            
            finput_matrix=tinv(finput_matrix)
            finput_matrix = fftshift(fft(input_matrix,dim=-2),dim=-2)/n_width
    ...
    def _gen_toeplitz2d_td(
        ...
        if method == "FFT":
            # compute fourier coefficents of input_matrix for only the last two dimensions
                ...
                """
                finput_matrix = fftshift(fftshift(fft2(1/input_matrix), dim=-2), dim=-1) / (n_height * n_width)
                
                finput_matrix = fftshift(fft(1/input_matrix,dim=-2))
                finput_matrix =tinv(finput_matrix)
                finput_matrix = fftshift(fft(input_matrix,dim=-1))
                finput_matrix=finput_matrix/(n_height * n_width)
                ...
                if input_matrix.ndim == 2:
                   print(torch.max(torch.abs(input_matrix))) 
                   print(torch.min(torch.abs(input_matrix))) 
                   print((input_matrix.shape))
                   print((input_matrix))
                   print(torch.std(input_matrix,dim=-1))
                   print(torch.std(input_matrix,dim=-2))
                   
                   finput_matrix = fftshift(fft(1/input_matrix,dim=-1),dim=-1)/n_width
                   
                   print((finput_matrix.shape))
                   print((finput_matrix))
                   print(torch.linalg.det(finput_matrix))
                   print(torch.std(finput_matrix,dim=-1))
                   print(torch.std(finput_matrix,dim=-2))
                   print(torch.max(torch.abs(finput_matrix))) 
                   print(torch.min(torch.abs(finput_matrix)))
                   
                   finput_matrix=tinv(finput_matrix)#[p_0 - pf_t, q_0 - qf_t])
                   
                   print((finput_matrix.shape))
                   
                   finput_matrix = fftshift(fft(finput_matrix,dim=-2),dim=-2)/n_height
                ...
                iepsy_fft = fftshift(fft(1.0 / input_matrix, dim=-2), dim=-2) / n_height

    # MATLAB: i_iepsy_nl = zeros(D_x,N_y,N_y)
                dtype=iepsy_fft.dtype
                i_iepsy_nl = torch.zeros( nharmonic_2,n_width, nharmonic_2,dtype=dtype, device=input_matrix.device)

    # MATLAB 索引换算: D_y/2+2-N_y : D_y/2+N_y
                y_start = int(n_height // 2 + 1 - nharmonic_2)
                y_end = int(n_height // 2 + nharmonic_2)

    # -------------------------------------------------------
    # Step 2: Construct Toeplitz matrices and invert each
    # -------------------------------------------------------
                for pp in range(n_width):
                        iepsy_n = iepsy_fft[ y_start:y_end,pp]
                        #print(iepsy_n.shape)
                        c = iepsy_n[nharmonic_2 - 1:2 * nharmonic_2-1]
                        #print(c.shape)
                        r = torch.flip(iepsy_n[:nharmonic_2], dims=[0])
                        #print(r.shape)
                        toeplitz_nl = toeplitz_torch(c, r)
                        i_iepsy_nl[ :, pp,:] = torch.linalg.inv(toeplitz_nl)

    # -------------------------------------------------------
    # Step 3: FFT along x-axis (width)
    # MATLAB: epsyx_fft = fftshift(fft(i_iepsy_nl,[],1),1)/D_x;
    # -------------------------------------------------------
                epsyx_fft = fftshift(fft(i_iepsy_nl, dim=-2), dim=-2) / n_width

    # MATLAB: epsyx_mnl = epsyx_fft(D_x/2+2-N_x:D_x/2+N_x,:,:);
                x_start = int(n_width // 2 + 1 - nharmonic_1)
                x_end = int(n_width // 2 + nharmonic_1)
                epsyx_mnl = epsyx_fft[:, x_start:x_end, :]
    ...
    # -------------------------------------------------------
                ...
                for rr in range(nharmonic_2):
                  for ss in range(nharmonic_2):
                          c = epsyx_mnl[ rr, nharmonic_1 - 1:2 * nharmonic_1 - 1,ss]
                          r = torch.flip(epsyx_mnl[rr,:nharmonic_1,  ss], dims=[0])
                          E4[ rr, :, ss,:] = toeplitz_torch(c, r)

    # -------------------------------------------------------
    # Step 5: Reshape to final 2D matrix
    # MATLAB: EPSyx(:,:) = reshape(E4,[N_x*N_y,N_x*N_y]);
    # -------------------------------------------------------
                EPSyx = E4.reshape(nharmonic_1 * nharmonic_2, nharmonic_1 * nharmonic_2)
                if input_matrix.ndim == 2:  
                  return EPSyx
                else:
                  return None
