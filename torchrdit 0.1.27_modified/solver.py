...
def create_solver(
    ...
    Args:
        ...
        fff_vector_scheme (str): Tangent vector field scheme to use when generating
                ...
                * ``'POL'`` 鈥? polarization-preserving tangents normalized by the global maximum.
                * ``'NORMAL'`` 鈥? per-pixel unit tangents for geometry-driven factorization.
                * ``'JONES'`` 鈥? convert refined tangents into Jones vectors after the solve.
                * ``'JONES_DIRECT'`` 鈥? run the Newton refinement directly in Jones space
                  (fmmax/S4 style).
    ...
    solver = create_solver(
        ...
        lam0=np.array([1.55]),  # Wavelength (渭m)
    ...
    print(f"Best angle: {sources[best_idx]['theta'] * 180/np.pi:.1f}掳")
    ```
...
class FourierBaseSolver(Cell3D, SolverSubjectMixin):
    ...
    def __init__(
        ...
        fff_smoothness_weight: float = 1e-3,
        fff_vector_steps: int = 1,
        is_use_Li: bool = False,  # if use Li's double Fourier transformation
        precision: Precision = Precision.SINGLE,
        device: Union[str, torch.device] = "cpu",
    ...
    def add_source(self, theta: float, phi: float, pte: float, ptm: float, norm_te_dir: str = "y") -> dict:
        ...
        - theta: Polar angle measured from the z-axis (0掳 = normal incidence)
        ...
        ```python
        import numpy as np
        # Create source with circular polarization (90掳 phase shift between components)
        source = solver.add_source(
            theta=0,
            phi=0,
            pte=1.0,
            ptm=1.0j  # 90掳 phase shift
        )
        ```
    ...
    def _process_layer(self, n_layer, matrices):
        ...
        if not layer.is_homogeneous:
            ...
            if layer.is_dispersive:
                ...
                toeplitz_er_td = layer.kermat_td
                toeplitz_er_dt = layer.kermat_dt                
                ...
                if toeplitz_er.dim() == 3 and n_sources > 1:
                    ...
                    toeplitz_er_td = toeplitz_er_td.unsqueeze(0).expand(n_sources, -1, -1, -1)
                    toeplitz_er_dt = toeplitz_er_dt.unsqueeze(0).expand(n_sources, -1, -1, -1)
            else:
                toeplitz_er = self.expand_dims(layer.kermat)
                toeplitz_er_td = self.expand_dims(layer.kermat_td)
                toeplitz_er_dt = self.expand_dims(layer.kermat_dt)
                if toeplitz_er.dim() == 3 and n_sources > 1:
                    toeplitz_er = toeplitz_er.unsqueeze(0).expand(n_sources, -1, -1, -1)
                    toeplitz_er_td = toeplitz_er_td.unsqueeze(0).expand(n_sources, -1, -1, -1)
                    toeplitz_er_dt = toeplitz_er_dt.unsqueeze(0).expand(n_sources, -1, -1, -1)
            ...
            if self.is_use_FFF:
                ...
                q_mat_i = blockmat2x2(
                    [
                        [
                            mat_kx_diag @ solve_tur_mky - delta_toeplitz_er @ p_yx,
                            toeplitz_er - mat_kx_diag @ solve_tur_mkx - delta_toeplitz_er @ p_yy,
                        ],
                        [
                            mat_ky_diag @ solve_tur_mky - toeplitz_er + delta_toeplitz_er @ p_xx,
                            -mat_ky_diag @ solve_tur_mkx+delta_toeplitz_er @ p_xy,
                        ],
                    ]
                )
                '''
                delExx=0.5*(delta_toeplitz_er @ p_xx+ p_xx@ delta_toeplitz_er)
                delEyy=0.5*(delta_toeplitz_er @ p_yy+ p_yy@ delta_toeplitz_er)
                delExy=0.5*(delta_toeplitz_er @ p_xy+ p_xy@ delta_toeplitz_er)
                delEyx=0.5*(delta_toeplitz_er @ p_yx+ p_yx@ delta_toeplitz_er)
                ...
                q_mat_i = blockmat2x2(
                    [
                        [
                            mat_kx_diag @ solve_tur_mky - delExy,
                            toeplitz_er - mat_kx_diag @ solve_tur_mkx -delEyy,
                        ],
                        [
                            mat_ky_diag @ solve_tur_mky - toeplitz_er + delExx,
                            delEyx - mat_ky_diag @ solve_tur_mkx,
                        ],
                    ]
                )
                '''
                ...
                if not(self.is_use_Li):                    
                    ...
                    q_mat_i = blockmat2x2(
                        [
                            [mat_kx_diag @ solve_tur_mky, toeplitz_er_td - mat_kx_diag @ solve_tur_mkx],
                            [mat_ky_diag @ solve_tur_mky - toeplitz_er_dt, -mat_ky_diag @ solve_tur_mkx],
                        ]
                    )
            ...
            if n_sources > 1:
                # Flatten (n_sources, n_freqs, M, N) 鈫? (n_sources*n_freqs, M, N) for algorithm processing
                original_shape = p_mat_i.shape  # Store for reshaping outputs
                batch_size = n_sources * p_mat_i.shape[1]  # n_sources * n_freqs
                matrix_shape = p_mat_i.shape[2:]  # (2*kdim虏, 2*kdim虏)
                ...
                # Flatten k_0 to match: (n_freqs,) 鈫? (n_sources*n_freqs,)
                ...
                smat_layer = self._solve_nonhomo_layer(
                    layer_thickness=slice_thickness,
                    p_mat_i=p_mat_i[0],  # Shape: (n_freqs, 2*kdim虏, 2*kdim虏)
                    q_mat_i=q_mat_i[0],  # Shape: (n_freqs, 2*kdim虏, 2*kdim虏)
    ...
    def solve(self, source: Union[dict, List[dict]], **kwargs) -> SolverResults:
        ...
        result_30deg = results[1]  # SolverResults for 30掳 incidence
        ...
        for i, result in enumerate(results):
            print(f"Angle {sources[i]['theta']*180/np.pi:.0f}掳: T={result.transmission[0]:.3f}")
        ```
        ...
        if isinstance(bg_material, str) and bg_material.lower() == "air":
            ...
            if self._matlib[bg_material].er.ndim == 0:
                self.layer_manager.layers[layer_index].ermat = (
                    self._matlib[bg_material].er * (1 - mask_format) + (er_bg - 0) * mask_format
                )
            else:
                self.layer_manager.layers[layer_index].ermat = (
                    self._matlib[bg_material].er[:, None, None] * (1 - mask_format) + (er_bg - 0) * mask_format
                )
    ...
    def _calculate_vector_field(self, mask: torch.Tensor, *, layer_index: Optional[int] = None):
        ...
        # print(tx.dtype)
        # print(ty.dtype)
        # Note that (tx,ty) is tangent field
        # but here normal vector field is needed here
        # Fortunately, (nx,ny)=(-ty*,tx*)
        ...
        weight_xx = (ty_mag_sq * inv_denom).to(dtype=self.tcomplex)
        weight_yy = (tx_mag_sq * inv_denom).to(dtype=self.tcomplex)
        weight_xy = -(ty_conj * tx) * inv_denom_complex#-torch.abs(tx * ty_conj) * inv_denom_complex#
...
class RCWASolver(FourierBaseSolver):
    ...
    def set_rdit_Norm_M(self, rdit_Norm_M):
        """Set the order of the R-DIT algorithm.
        ...
        ```
        """
        self._algorithm.set_rdit_Norm_M(rdit_Norm_M)
...
class RDITSolver(FourierBaseSolver):
    ...
    achieving up to 16.2脳 speedup compared to traditional RCWA-based methods.
    ...
    def set_rdit_Norm_M(self, rdit_Norm_M: int) -> None:
        """Set the order of the R-DIT algorithm.
